...
func GetKaryawanFromPhoneNumber(phonenumber string) (staf Presensi) {
	presensi := MongoConnect("adorable").Collection("presensi")
	filter := bson.M{"phone_number": phonenumber}
	err := presensi.FindOne(context.TODO(), filter).Decode(&staf)
	if err != nil {
